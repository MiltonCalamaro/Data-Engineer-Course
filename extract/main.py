...
is_root_path = re.compile(r'^/.+$') ### que inicie con "/" con al menos una letra y que termine
									### some-text
def _news_scrapper(news_site_uid):
    ...
    for link in homepage.article_links:
        ...
        if article:
            ...
            articles.append([article.body,article.title,link])
            #articles.append(article)
            #if len(articles)==5:
            #    break
    #_save_articles(news_site_uid, articles)
    df=pd.DataFrame(articles,columns=["body","title","url"])
    df.to_csv("{}_articles.csv".format(news_site_uid),index=False)
